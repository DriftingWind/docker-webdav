...
2. 在`dav.conf`中声明UTF-8，解决文件命令乱码。
3. 使用GitHub Actions编译，修改映像名称以便区分。
4. 用容器中包含的`htpasswd`工具为指定用户名生成密码hash并输出到终端：
   ```bash
   sudo docker run -it --rm ghcr.io/driftingwind/windwebdav /bin/bash -c "htpasswd -B -c tempfile myusername && cat tempfile"
   ```
...
5. 参考`docker-compose.yml`
    ...
    services:
      app:
        image: ghcr.io/driftingwind/windwebdav
